...
# Source: https://github.com/ClownsharkBatwing/RES4LYF/blob/119679d8d8d26e6db52757e705488abb6399d7d4/latents.py
...
def hard_light_blend(base_latent, blend_latent):
    ...
    positive_result = torch.where(
        blend_latent < 0.5, 2 * positive_latent * blend_latent, 1 - 2 * (1 - positive_latent) * (1 - blend_latent)
    )

    negative_result = torch.where(
        blend_latent < 0.5,
...
def untile_latent(
    tiles: torch.Tensor, orig_shape: Tuple[int, ...], tile_hw: Tuple[int, int], positions: Tuple[List[int], List[int]]
) -> torch.Tensor:
    """
