...
# Source: https://github.com/ClownsharkBatwing/RES4LYF/blob/119679d8d8d26e6db52757e705488abb6399d7d4/beta/rk_noise_sampler_beta.py
...
class RK_NoiseSampler:
    ...
    def init_noise_samplers(
        ...
        if noise_sampler_type == "fractal":
            self.noise_sampler = NOISE_GENERATOR_CLASSES.get(noise_sampler_type)(
                x=x, seed=seed, sigma_min=self.sigma_min, sigma_max=self.sigma_max
            )
            self.noise_sampler.alpha = alpha
            self.noise_sampler.k = k
            self.noise_sampler.scale = scale
        if noise_sampler_type2 == "fractal":
            self.noise_sampler2 = NOISE_GENERATOR_CLASSES.get(noise_sampler_type2)(
                x=x, seed=noise_seed_substep, sigma_min=self.sigma_min, sigma_max=self.sigma_max
            )
            self.noise_sampler2.alpha = alpha2
            self.noise_sampler2.k = k2
            self.noise_sampler2.scale = scale2
        else:
            self.noise_sampler = NOISE_GENERATOR_CLASSES_SIMPLE.get(noise_sampler_type)(
                x=x, seed=seed, sigma_min=self.sigma_min, sigma_max=self.sigma_max
            )
            self.noise_sampler2 = NOISE_GENERATOR_CLASSES_SIMPLE.get(noise_sampler_type2)(
                x=x, seed=noise_seed_substep, sigma_min=self.sigma_min, sigma_max=self.sigma_max
            )
    ...
    def get_sde_coeff(
        self, sigma_next: Tensor, sigma_down: Tensor = None, sigma_up: Tensor = None, eta: float = 0.0, VP_OVERRIDE=None
        ...
        if VARIANCE_PRESERVING:
            ...
            if sigma_down is not None:
                sigma_up = (
                    sigma_next**2 - sigma_down**2
                ) ** 0.5  # not sure this is correct           #TODO: CHECK THIS
            elif sigma_up is not None:
    ...
    def set_sde_step(self, sigma: Tensor, sigma_next: Tensor, eta: float, overshoot: float, s_noise: float) -> None:
        ...
        self.sigma_up_eta, self.sigma_eta, self.sigma_down_eta, self.alpha_ratio_eta = self.get_sde_step(
            sigma, sigma_next, eta, self.noise_mode_sde, self.DOWN_STEP, SUBSTEP=False
        )

        self.sigma_up, self.sigma, self.sigma_down, self.alpha_ratio = self.get_sde_step(
            sigma, sigma_next, overshoot, self.overshoot_mode, self.DOWN_STEP, SUBSTEP=False
        )
    ...
    def set_sde_substep(
        ...
        self.sub_sigma_down_eta = self.sub_sigma_down = self.sub_sigma_next = self.s_[
            row + self.row_offset + multistep_stages
        ]
        ...
        if row < self.rows and self.s_[row + self.row_offset + multistep_stages] > 0:
            if diag_iter > 0 and diag_iter == implicit_steps_diag and self.EO("implicit_substep_skip_final_eta"):
            ...
            elif (row < self.rows - self.row_offset - multistep_stages or diag_iter < implicit_steps_diag) or self.EO(
                "substep_eta_use_final"
            ):
                self.sub_sigma_up, self.sub_sigma, self.sub_sigma_down, self.sub_alpha_ratio = self.get_sde_substep(
                    sigma=self.s_[row],
                ...
                self.sub_sigma_up_eta, self.sub_sigma_eta, self.sub_sigma_down_eta, self.sub_alpha_ratio_eta = (
                    self.get_sde_substep(
                        sigma=self.s_[row],
                        sigma_next=self.s_[row + self.row_offset + multistep_stages],
                        eta=eta_substep,
        ...
        if self.h_fn(self.sub_sigma_next, self.sigma) != 0:
            self.h_new = (
                self.h * self.h_fn(self.sub_sigma_down, self.sigma) / self.h_fn(self.sub_sigma_next, self.sigma)
            )
            self.h_eta = (
                self.h * self.h_fn(self.sub_sigma_down_eta, self.sigma) / self.h_fn(self.sub_sigma_next, self.sigma)
            )
    ...
    def get_sde_substep(
        ...
        return self.get_sde_step(
            sigma=sigma,
            sigma_next=sigma_next,
            eta=eta,
            noise_mode_override=noise_mode_override,
            DOWN=DOWN,
            SUBSTEP=True,
        )
    ...
    def get_sde_step(
        ...
        if (
            DOWN
        ):  # calculates noise level by first scaling sigma_down from sigma_next, instead of sigma_up from sigma_next
    ...
    def linear_noise_init(
        self,
        y: Tensor,
        sigma_curr: Tensor,
        x_base: Optional[Tensor] = None,
        x_curr: Optional[Tensor] = None,
    ...
    def linear_noise_step(
        self,
        y: Tensor,
        sigma_curr: Optional[Tensor] = None,
        x_base: Optional[Tensor] = None,
        x_curr: Optional[Tensor] = None,
    ...
    def swap_noise_step(
        ...
        x_noised = (
            self.alpha_ratio_eta * (denoised_next + self.sigma_down_eta * eps_next)
            + self.sigma_up_eta * noise * self.s_noise
        )
    ...
    def swap_noise_substep(
        ...
        x_noised = (
            self.sub_alpha_ratio_eta * (denoised_next + self.sub_sigma_down_eta * eps_next)
            + self.sub_sigma_up_eta * noise * self.s_noise_substep
        )
    ...
    def swap_noise_inv_substep(
        self,
        x_0: Tensor,
        x_next: Tensor,
        eta_substep: float,
        row: int,
        row_offset_multistep_stages: int,
        brownian_sigma: Optional[Tensor] = None,
        brownian_sigma_next: Optional[Tensor] = None,
        mask: Optional[Tensor] = None,
        guide: Optional[Tensor] = None,
        ...
        sub_sigma_up, sub_sigma, sub_sigma_down, sub_alpha_ratio = self.get_sde_substep(
            sigma=1 - self.s_[row],
        ...
        x_noised = (
            sub_alpha_ratio * (denoised_next + sub_sigma_down * eps_next) + sub_sigma_up * noise * self.s_noise_substep
        )
    ...
    def sigma_from_to(
        self, x_0: Tensor, x_down: Tensor, sigma: Tensor, sigma_down: Tensor, sigma_next: Tensor
    ) -> Tensor:  # sigma, sigma_from, sigma_to
    ...
    def prepare_sigmas(
        self, sigmas: Tensor, sigmas_override: Tensor, d_noise: float, d_noise_start_step: int, sampler_mode: str
    ) -> Tuple[Tensor, bool]:
        ...
        if (
            sigmas[0] == 0.0
        ):  # remove padding used to prevent comfy from adding noise to the latent (for unsampling, etc.)
            UNSAMPLE = True
            if sigmas[-1] == 0.0:
