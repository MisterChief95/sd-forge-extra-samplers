...
# Source: https://github.com/ClownsharkBatwing/RES4LYF/blob/119679d8d8d26e6db52757e705488abb6399d7d4/beta/rk_method_beta.py
...
class RK_Method_Beta:
    ...
    def model_epsilon(self, x: Tensor, sigma: Tensor, **extra_args) -> Tuple[Tensor, Tensor]:
        ...
        eps = (x - denoised) / (sigma * s_in).view(
            x.shape[0], 1, 1, 1
    ...
    def model_denoised(self, x: Tensor, sigma: Tensor, **extra_args) -> Tensor:
        ...
        if self.EO("tile_model_calls"):
            ...
            if negative_control is not None and hasattr(negative_control, "cond_hint_original"):
                negative_cond_hint_init = (
                    negative_control.cond_hint.clone() if negative_control.cond_hint is not None else None
                )
            ...
            if positive_control is not None and hasattr(positive_control, "cond_hint_original"):
                positive_cond_hint_init = (
                    positive_control.cond_hint.clone() if positive_control.cond_hint is not None else None
                )
                if (
                    ...
                    positive_control.cond_hint_original = positive_control_pretile.to(
                        positive_control.cond_hint_original
                    )
                positive_control_pretile = positive_control.cond_hint_original.clone().to(torch.float16).to("cuda")
                control_tiles, control_orig_shape, control_grid, control_strides = tile_latent(
                    positive_control_pretile, tile_size=(tile_h_full, tile_w_full)
                )
            ...
            for i in range(tiles.shape[0]):
                ...
                if y0_style_pos is not None:
                    self.extra_args["model_options"]["transformer_options"]["y0_style_pos"] = y0_style_pos_tiles[
                        i
                    ].unsqueeze(0)
                if y0_style_neg is not None:
                    self.extra_args["model_options"]["transformer_options"]["y0_style_neg"] = y0_style_neg_tiles[
                        i
                    ].unsqueeze(0)
    ...
    def update_transformer_options(
        self,
        transformer_options: Optional[dict] = None,
    ):

        self.extra_args.setdefault("model_options", {}).setdefault("transformer_options", {}).update(
            transformer_options
        )
        return
    ...
    def set_coeff(
        ...
        h_prev = []
        a, b, u, v, ci, multistep_stages, hybrid_stages, FSAL = get_rk_methods_beta(
            rk_type,
            h,
            c1,
            c2,
    ...
    def update_substep(
        ...
        if self.SYNC_SUBSTEP_MEAN_CW and lying_eps_row_factor != 1.0:
            x_[row_tmp_offset] = (
                x_[row_tmp_offset]
                - x_[row_tmp_offset].mean(dim=(-2, -1), keepdim=True)
                + x_orig_row.mean(dim=(-2, -1), keepdim=True)
            )
        ...
        if (self.SYNC_SUBSTEP_MEAN_CW and h_new != h_new_orig) or self.EO("sync_mean_noise"):
            if not self.EO("disable_sync_mean_noise"):
                ...
                x_[row_tmp_offset] = (
                    x_[row_tmp_offset]
                    - x_[row_tmp_offset].mean(dim=(-2, -1), keepdim=True)
                    + x_row_down.mean(dim=(-2, -1), keepdim=True)
                )
    ...
    def u_k_einsum(self, row: int, k_prev: Tensor) -> Tensor:
        return (
            torch.einsum("i, i... -> ...", self.U[row], k_prev[: self.cols])
            if (self.U is not None and k_prev is not None)
            else 0
        )

    def v_k_einsum(self, row: int, k_prev: Tensor) -> Tensor:
        return (
            torch.einsum("i, i... -> ...", self.V[row], k_prev[: self.cols])
            if (self.V is not None and k_prev is not None)
            else 0
        )
    ...
    def zum_tableau(
        ...
        u_k_sum = (
            torch.einsum("ij, j... -> i...", self.U, k_prev[: self.cols])
            if (self.U is not None and k_prev is not None)
            else 0
        )
    ...
    def swap_rk_type_at_step_or_threshold(
        self,
        x_0: Tensor,
        data_prev_: Tensor,
        NS,
    ...
    def bong_iter(
        ...
        if (
            self.EO("bong_start_step", 0) > step
            or step > self.EO("bong_stop_step", 10000)
            or (self.unsample_bongmath == False and s_[-1] > s_[0])
        ):
            return x_0, x_, eps_
                        ...
                        if BONGMATH_Y and not self.EO("disable_bongmath_y"):
                            if self.EXPONENTIAL:
                                ...
                                if self.VE_MODEL:
                                    eps_ = (
                                        sync_mask * eps_x_
                                        + (1 - sync_mask) * eps_x2y_
                                        + weight_mask * (-eps_y_ + sigma * (-noise_sync))
                                    )
                                    if self.EO("sync_x2y"):
                                        eps_ = (
                                            sync_mask * eps_x_
                                            + (1 - sync_mask) * eps_x2y_
                                            + weight_mask * (-eps_x2y_ + sigma * (-noise_sync))
                                        )
                                else:
                                    eps_ = (
                                        sync_mask * eps_x_
                                        + (1 - sync_mask) * eps_x2y_
                                        + weight_mask * (-eps_y_ + sigma * (y0_bongflow - noise_sync))
                                    )
                                    if self.EO("sync_x2y"):
                                        eps_ = (
                                            sync_mask * eps_x_
                                            + (1 - sync_mask) * eps_x2y_
                                            + weight_mask * (-eps_x2y_ + sigma * (y0_bongflow - noise_sync))
                                        )
                            else:
                                eps_x_[: s_.shape[0]] = (x_[: s_.shape[0]] - data_x_[: s_.shape[0]]) / s_.view(
                                    -1, 1, 1, 1, 1
                                )  # or should it be vs x_0???
                                eps_x2y_ = torch.zeros_like(eps_x_)
                                eps_x2y_[: s_.shape[0]] = (x_[: s_.shape[0]] - data_y_[: s_.shape[0]]) / s_.view(
                                    -1, 1, 1, 1, 1
                                )  # or should it be vs x_0???
                                ...
                                if self.VE_MODEL:
                                    eps_ = (
                                        sync_mask * eps_x_
                                        + (1 - sync_mask) * eps_x2y_
                                        + weight_mask * (noise_sync - eps_y_)
                                    )
                                    if self.EO("sync_x2y"):
                                        eps_ = (
                                            sync_mask * eps_x_
                                            + (1 - sync_mask) * eps_x2y_
                                            + weight_mask * (noise_sync - eps_x2y_)
                                        )
                                else:
                                    eps_ = (
                                        sync_mask * eps_x_
                                        + (1 - sync_mask) * eps_x2y_
                                        + weight_mask * (noise_sync - eps_y_ - y0_bongflow)
                                    )
                                    if self.EO("sync_x2y"):
                                        eps_ = (
                                            sync_mask * eps_x_
                                            + (1 - sync_mask) * eps_x2y_
                                            + weight_mask * (noise_sync - eps_x2y_ - y0_bongflow)
                                        )
...
class RK_Method_Exponential(RK_Method_Beta):
    ...
    def __call__(
        ...
        denoised = self.model_denoised(x.to(self.model_device), sub_sigma.to(self.model_device), **self.extra_args).to(
            sigma.device
        )
