...
# Source: https://github.com/ClownsharkBatwing/RES4LYF/blob/119679d8d8d26e6db52757e705488abb6399d7d4/beta/rk_sampler_beta.py
...
def init_implicit_sampling(
    ...
    if EO("implicit_skip_model_call_at_start") and denoised.sum() + eps.sum() != 0:
        ...
        if denoised_prev2.sum() == 0:
            ...
            for r in range(RK.rows):
                sratio = sigma - s_[r]
                data_[r] = (
                    lagrange_interpolation([0, 1], [denoised_prev2, denoised], 1 + w * RK.C[r]).squeeze(0)
                    + denoised_prev2
                    - denoised
                )
                eps_[r] = RK.get_epsilon_anchored(x_0, data_[r], s_[r])
    ...
    if not EO(
        (
            "implicit_lagrange_init",
            "radaucycle",
            "implicit_full_skip_model_call_at_start",
            "implicit_lagrange_skip_model_call_at_start",
        )
    ):
        for r in range(RK.rows):
...
def sample_rk_beta(
    ...
    if "end_step" in state_info and (sampler_mode == "resample" or sampler_mode == "unsample"):
        if (
            state_info["completed"] != True
            and state_info["end_step"] != 0
            and state_info["end_step"] != -1
            and state_info["end_step"] < len(state_info["sigmas"]) - 1
        ):  # incomplete run in previous sampler node
            if (
                state_info["sampler_mode"] in {"standard", "resample"}
                and sampler_mode == "unsample"
                and sigmas[2] < sigmas[1]
            ):
    ...
    pseudoimplicit_row_weights = EO("pseudoimplicit_row_weights", [1.0 for _ in range(100)])
    pseudoimplicit_step_weights = EO(
        "pseudoimplicit_step_weights", [1.0 for _ in range(max(implicit_steps_diag, implicit_steps_full) + 1)]
    )
    ...
    RK.extra_args.setdefault("model_options", {}).setdefault("transformer_options", {})
    ...
    RK.unsample_bongmath = (
        BONGMATH if unsample_bongmath is None else unsample_bongmath
    )  # allow turning off bongmath for unsampling with cycles
    ...
    NS.init_noise_samplers(
        x,
        noise_seed,
        noise_seed_substep,
        noise_sampler_type,
        noise_sampler_type_substep,
        noise_mode_sde,
        ...
        k_substep,
        last_rng=last_rng,
        last_rng_substep=last_rng_substep,
    )
    ...
    x = LG.init_guides(
        x, RK.IMPLICIT, guides, NS.noise_sampler, batch_num, sigmas[step], guide_inversion_y0, guide_inversion_y0_inv
    )
    ...
    if (LG.mask != 1.0).any() and (
        (LG.y0 == 0).all() or (LG.y0_inv == 0).all()
    ):  #  and   not LG.guide_mode.startswith("flow"):  # (LG.y0.sum() == 0 or LG.y0_inv.sum() == 0):
        SKIP_PSEUDO = True
        ...
        SKIP_PSEUDO = False
    if (
        guides is not None and guides.get("guide_mode", "") != "inversion" or sampler_mode != "unsample"
    ):  # do not set denoised_prev to noise guide with inversion!
    ...
    if noise_initial is not None:
        ...
        RK.update_transformer_options(
            {"x_init": x_init._copy() if hasattr(x_init, "is_nested") and x_init.is_nested else x_init.clone()}
        )
    ...
    if RENOISE:  # TODO: adapt for noise inversion somehow
        if VE_MODEL:
            ...
            x = (1 - sigmas[step]) * x + sigmas[step] * NS.noise_sampler(
    ...
    while step < num_steps:
        ...
        SYNC_GUIDE_ACTIVE = LG.guide_mode.startswith("sync") and (
            LG.lgw[step_sched] != 0
            or LG.lgw_inv[step_sched] != 0
            or LG.lgw_sync[step_sched] != 0
            or LG.lgw_sync_inv[step_sched] != 0
        )
            ...
            if LG.HAS_LATENT_GUIDE_ADAIN:
                if LG.lgw_adain[step_sched] == 0.0:
                    ...
                    if "blocks_adain_mmdit" in guides:
                        blocks_adain = {
                            "double_weights": [
                                val * LG.lgw_adain[step_sched] for val in guides["blocks_adain_mmdit"]["double_weights"]
                            ],
                            "single_weights": [
                                val * LG.lgw_adain[step_sched] for val in guides["blocks_adain_mmdit"]["single_weights"]
                            ],
            ...
            if LG.HAS_LATENT_GUIDE_ATTNINJ:
                if LG.lgw_attninj[step_sched] == 0.0:
                    ...
                    if "blocks_attninj_mmdit" in guides:
                        blocks_attninj = {
                            "double_weights": [
                                val * LG.lgw_attninj[step_sched]
                                for val in guides["blocks_attninj_mmdit"]["double_weights"]
                            ],
                            "single_weights": [
                                val * LG.lgw_attninj[step_sched]
                                for val in guides["blocks_attninj_mmdit"]["single_weights"]
                            ],
        ...
        if AttnMask is not None:
            ...
            RK.extra_args["model_options"]["transformer_options"]["regional_conditioning_weight"] = (
                regional_conditioning_weights[step_sched]
            )
            RK.extra_args["model_options"]["transformer_options"]["regional_conditioning_floor"] = (
                regional_conditioning_floors[step_sched]
            )
        ...
        noise_scaling_eta = (
            noise_scaling_etas[step_sched].to(x) if noise_scaling_etas is not None else noise_scaling_eta
        )
        noise_scaling_weight = (
            noise_scaling_weights[step_sched].to(x) if noise_scaling_weights is not None else noise_scaling_weight
        )
        ...
        if (noise_scaling_eta > 0 or noise_scaling_weight != 0) and noise_scaling_type != "model_d":
            ...
            if noise_scaling_type in {"sampler", "model", "model_alpha"}:
                ...
                if RK.multistep_stages > 0:  # hardcoded s_[1] for multistep samplers, which are never multistage
                    lying_su, lying_sigma, lying_sd, lying_alpha_ratio = NS.get_sde_step(
                        NS.s_[1] / sigma_divisor,
                        NS.s_[0] / sigma_divisor,
                        noise_scaling_eta,
                        noise_scaling_mode,
        ...
        if INIT_SAMPLE_LOOP:
            ...
            x_, data_, eps_, eps_prev_ = (
                torch.zeros(RK.rows + 2, *x.shape, dtype=default_dtype, device=work_device) for _ in range(4)
            )
            ...
            if sampler_mode in {"unsample", "resample"}:
                ...
                if data_prev_ is not None:
                    ...
                    data_prev_ = torch.zeros(
                        4, *x.shape, dtype=default_dtype, device=work_device
                    )  # multistep max is 4m... so 4 needed
            else:
                data_prev_ = torch.zeros(
                    4, *x.shape, dtype=default_dtype, device=work_device
                )  # multistep max is 4m... so 4 needed
        ...
        if RK.rows + 2 > x_.shape[0]:
            ...
            x_gap_, data_gap_, eps_gap_, eps_prev_gap_ = (
                torch.zeros(row_gap, *x.shape, dtype=default_dtype, device=work_device) for _ in range(4)
            )
                    ...
                    if RK.EXPONENTIAL:
                        if VE_MODEL:
                            eps_[ms] = (
                                sync_mask * eps_x
                                + (1 - sync_mask) * eps_x2y
                                + weight_mask * (-eps_y + sigma * (-noise_bongflow))
                            )
                            if EO("sync_x2y"):
                                eps_[ms] = (
                                    sync_mask * eps_x
                                    + (1 - sync_mask) * eps_x2y
                                    + weight_mask * (-eps_x2y + sigma * (-noise_bongflow))
                                )
                        else:
                            eps_[ms] = (
                                sync_mask * eps_x
                                + (1 - sync_mask) * eps_x2y
                                + weight_mask * (-eps_y + sigma * (y0_bongflow - noise_bongflow))
                            )
                            if EO("sync_x2y"):
                                eps_[ms] = (
                                    sync_mask * eps_x
                                    + (1 - sync_mask) * eps_x2y
                                    + weight_mask * (-eps_x2y + sigma * (y0_bongflow - noise_bongflow))
                                )
                    else:
                        if VE_MODEL:
                            eps_[ms] = (
                                sync_mask * eps_x
                                + (1 - sync_mask) * eps_x2y
                                + weight_mask * (-eps_y + (noise_bongflow))
                            )
                            if EO("sync_x2y"):
                                eps_[ms] = (
                                    sync_mask * eps_x
                                    + (1 - sync_mask) * eps_x2y
                                    + weight_mask * (-eps_x2y + (noise_bongflow))
                                )
                        else:
                            eps_[ms] = (
                                sync_mask * eps_x
                                + (1 - sync_mask) * eps_x2y
                                + weight_mask * (-eps_y + (noise_bongflow - y0_bongflow))
                            )
                            if EO("sync_x2y"):
                                eps_[ms] = (
                                    sync_mask * eps_x
                                    + (1 - sync_mask) * eps_x2y
                                    + weight_mask * (-eps_x2y + (noise_bongflow - y0_bongflow))
                                )
        ...
        if RK.IMPLICIT:
            x_, eps_, data_ = init_implicit_sampling(
                RK,
                x_0,
                x_,
                eps_,
        ...
        while full_iter < implicit_steps_full + 1:
            if RK.IMPLICIT:
                x_, eps_ = RK.newton_iter(
                    x_0, x_, eps_, eps_prev_, data_, NS.s_, 0, NS.h, sigmas, step, "init", SYNC_GUIDE_ACTIVE
                )
            ...
            if step > 0 or not SKIP_PSEUDO:
                ...
                x_0, x_, eps_ = LG.prepare_fully_pseudoimplicit_guides_substep(
                    x_0,
                    x_,
                    eps_,
                    eps_prev_,
            ...
            for row in range(RK.rows - RK.multistep_stages - RK.row_offset + 1):
                ...
                while diag_iter < implicit_steps_diag + 1:
                    ...
                    if row < RK.rows:
                        ...
                        if step > 0 or not SKIP_PSEUDO:
                            x_0, x_, eps_, x_row_pseudoimplicit, sub_sigma_pseudoimplicit = (
                                LG.process_pseudoimplicit_guides_substep(
                                    x_0,
                                    x_,
                                    eps_,
                                    eps_prev_,
                        ...
                        # PREPARE MODEL CALL
                        if (
                            LG.guide_mode in GUIDE_MODE_NAMES_PSEUDOIMPLICIT
                            and (step > 0 or not SKIP_PSEUDO)
                            and (LG.lgw[step_sched] > 0 or LG.lgw_inv[step_sched] > 0)
                            and x_row_pseudoimplicit is not None
                        ):
                        ...
                        elif (full_iter > 0 and RK.row_offset == 1 and row == 0) or (
                            full_iter > 0 and RK.row_offset == 0 and row == 0 and EO("fully_implicit_update_x")
                        ):
                            if EO("fully_explicit_pogostick_eta"):
                                super_alpha_ratio, super_sigma_down, super_sigma_up = NS.get_sde_coeff(
                                    sigma, sigma_next, None, eta
                                )
                                x = super_alpha_ratio * x + super_sigma_up * NS.noise_sampler(
                                ...
                                implicit_type == "rebound"
                            ):  # TODO: ADAPT REBOUND IMPLICIT TO WORK WITH FLOW GUIDE MODE
                        ...
                        # All others
                            ...
                            if diag_iter > 0:  # Diagonally implicit iteration (explicit or implicit)
                                if EO("diag_explicit_pogostick_eta"):
                                    super_alpha_ratio, super_sigma_down, super_sigma_up = NS.get_sde_coeff(
                                        NS.s_[row], NS.s_[row + RK.row_offset + RK.multistep_stages], None, eta
                                    )
                                    x_[row + RK.row_offset] = super_alpha_ratio * x_[
                                        row + RK.row_offset
                                    ] + super_sigma_up * NS.noise_sampler(
                                        sigma=NS.s_[row + RK.row_offset + RK.multistep_stages], sigma_next=NS.s_[row]
                                    )
                                    ...
                                    eps_[row], data_[row] = RK(
                                        x_[row + RK.row_offset],
                                        NS.s_[row + RK.row_offset + RK.multistep_stages],
                                        x_0,
                                        sigma,
                                    )

                                    x_ = RK.update_substep(
                                        x_0, x_, eps_, eps_prev_, row, RK.row_offset, NS.h_new, NS.h_new_orig
                                    )
                                    x_[row + RK.row_offset] = NS.rebound_overshoot_substep(x_0, x_[row + RK.row_offset])

                                    x_[row + RK.row_offset] = NS.sigma_from_to(
                                        x_0,
                                        x_[row + RK.row_offset],
                                        sigma,
                                        NS.s_[row + RK.row_offset + RK.multistep_stages],
                                ...
                                elif implicit_type_substeps == "retro-eta" and (
                                    NS.sub_sigma_up > 0 or NS.sub_sigma_up_eta > 0
                                ):
                                    x_tmp = NS.sigma_from_to(
                                        x_0,
                                        x_[row + RK.row_offset],
                                        sigma,
                                        NS.s_[row + RK.row_offset + RK.multistep_stages],
                                    ...
                                    and (NS.sub_sigma_up > 0 or NS.sub_sigma_up_eta > 0)
                                    and not EO("disable_diag_explicit_bongmath_rebound")
                                ):
                                    if BONGMATH:
                                        x_tmp = x_[row]
                                        s_tmp = NS.s_[row]
                            ...
                            if RK.IMPLICIT:
                                if not EO("disable_implicit_guide_preproc"):
                                    eps_, x_ = LG.process_guides_substep(
                                        x_0,
                                        x_,
                                        eps_,
                                        data_,
                                    ...
                                    eps_prev_, x_ = LG.process_guides_substep(
                                        x_0,
                                        x_,
                                        eps_prev_,
                                        data_,
                                ...
                                if row == 0 and (EO("implicit_lagrange_init") or EO("radaucycle")):
                                    ...
                                    x_[row + RK.row_offset] = x_0 + NS.h_new * RK.zum(
                                        row + RK.row_offset, eps_, eps_prev_
                                    )
                                    x_[row + RK.row_offset] = NS.rebound_overshoot_substep(x_0, x_[row + RK.row_offset])
                                    if row > 0:
                                        if not LG.guide_mode.startswith("flow") or (
                                            LG.lgw[step_sched] == 0
                                            and LG.lgw[step + 1] == 0
                                            and LG.lgw_inv[step_sched] == 0
                                            and LG.lgw_inv[step + 1] == 0
                                        ):
                                            x_row_tmp = NS.swap_noise_substep(
                                                x_0, x_[row + RK.row_offset], mask=sde_mask, guide=LG.y0
                                            )

                                            if LG.ADAIN_NOISE_MODE == "smart":  # _smartnoise_implicit"):
                                                data_next = denoised + NS.h_new * RK.zum(
                                                    row + RK.row_offset + RK.multistep_stages, data_, data_prev_
                                                )
                                                if VE_MODEL:
                                                    z_[row + RK.row_offset] = (x_row_tmp - data_next) / s_tmp
                                                else:
                                                    z_[row + RK.row_offset] = (
                                                        x_row_tmp - (NS.sigma_max - s_tmp) * data_next
                                                    ) / s_tmp
                                                RK.update_transformer_options({"z_": z_})

                                            if SYNC_GUIDE_ACTIVE:
                                                noise_bongflow_new = (
                                                    x_row_tmp - x_[row + RK.row_offset]
                                                ) / s_tmp + noise_bongflow
                                                yt_[row + RK.row_offset] += s_tmp * (
                                                    noise_bongflow_new - noise_bongflow
                                                )
                                                x_0 += sigma * (noise_bongflow_new - noise_bongflow)
                                                if not EO("disable_i_bong"):
                                                    for i_bong in range(len(NS.s_)):
                                                        x_[i_bong] += NS.s_[i_bong] * (
                                                            noise_bongflow_new - noise_bongflow
                                                        )
                                        ...
                                        if SYNC_GUIDE_ACTIVE:
                                            if VE_MODEL:
                                                yt_[: NS.s_.shape[0], 0] = y0_bongflow + NS.s_.view(
                                                    -1, *[1] * (x.ndim - 1)
                                                ) * (noise_bongflow)
                                                yt_0 = y0_bongflow + sigma * (noise_bongflow)
                                            else:
                                                yt_[: NS.s_.shape[0], 0] = y0_bongflow + NS.s_.view(
                                                    -1, *[1] * (x.ndim - 1)
                                                ) * (noise_bongflow - y0_bongflow)
                                                yt_0 = y0_bongflow + sigma * (noise_bongflow - y0_bongflow)

                                            if RK.EXPONENTIAL:
                                                eps_y_ = (
                                                    data_y_ - yt_0
                                                )  # yt_        # watch out for fuckery with size of tableau being smaller later in a chained sampler
                                            else:
                                                if BONGMATH:
                                                    eps_y_[: NS.s_.shape[0]] = (
                                                        yt_[: NS.s_.shape[0]] - data_y_[: NS.s_.shape[0]]
                                                    ) / NS.s_.view(-1, *[1] * (x_.ndim - 1))
                                                else:
                                                    eps_y_[: NS.s_.shape[0]] = (
                                                        yt_0.repeat(NS.s_.shape[0], *[1] * (x_.ndim - 1))
                                                        - data_y_[: NS.s_.shape[0]]
                                                    ) / sigma  # calc exact to c0 node
                                            if not BONGMATH:
                                                if RK.EXPONENTIAL:
                                                    eps_x_ = data_x_ - x_0
                                                else:
                                            ...
                                            for ms in range(len(eps_)):
                                                if RK.EXPONENTIAL:
                                                    if VE_MODEL:  # ZERO IS THIS                      # ONE IS THIS
                                                        eps_[ms] = (
                                                            sync_mask * eps_x_[ms]
                                                            + (1 - sync_mask) * eps_x2y_[ms]
                                                            + weight_mask * (-eps_y_[ms] + sigma * (-noise_bongflow))
                                                        )
                                                        if EO("sync_x2y"):
                                                            eps_[ms] = (
                                                                sync_mask * eps_x_[ms]
                                                                + (1 - sync_mask) * eps_x2y_[ms]
                                                                + weight_mask
                                                                * (-eps_x2y_[ms] + sigma * (-noise_bongflow))
                                                            )
                                                    else:
                                                        eps_[ms] = (
                                                            sync_mask * eps_x_[ms]
                                                            + (1 - sync_mask) * eps_x2y_[ms]
                                                            + weight_mask
                                                            * (-eps_y_[ms] + sigma * (y0_bongflow - noise_bongflow))
                                                        )
                                                        if EO("sync_x2y"):
                                                            eps_[ms] = (
                                                                sync_mask * eps_x_[ms]
                                                                ...
                                                                * (
                                                                    -eps_x2y_[ms]
                                                                    + sigma * (y0_bongflow - noise_bongflow)
                                                                )
                                                            )
                                                else:
                                                    if VE_MODEL:
                                                        eps_[ms] = (
                                                            sync_mask * eps_x_[ms]
                                                            + (1 - sync_mask) * eps_x2y_[ms]
                                                            + weight_mask * (-eps_y_[ms] + (noise_bongflow))
                                                        )
                                                        if EO("sync_x2y"):
                                                            eps_[ms] = (
                                                                sync_mask * eps_x_[ms]
                                                                + (1 - sync_mask) * eps_x2y_[ms]
                                                                + weight_mask * (-eps_x2y_[ms] + (noise_bongflow))
                                                            )
                                                    else:
                                                        eps_[ms] = (
                                                            sync_mask * eps_x_[ms]
                                                            + (1 - sync_mask) * eps_x2y_[ms]
                                                            + weight_mask
                                                            * (-eps_y_[ms] + (noise_bongflow - y0_bongflow))
                                                        )
                                                        if EO("sync_x2y"):
                                                            eps_[ms] = (
                                                                sync_mask * eps_x_[ms]
                                                                + (1 - sync_mask) * eps_x2y_[ms]
                                                                + weight_mask
                                                                * (-eps_x2y_[ms] + (noise_bongflow - y0_bongflow))
                                                            )

                                        if (
                                            BONGMATH
                                            and step < sigmas.shape[0] - 1
                                            and sigma > 0.03
                                            and not EO("disable_implicit_prebong")
                                        ):
                                            BONGMATH_Y = SYNC_GUIDE_ACTIVE

                                            x_0, x_, eps_ = RK.bong_iter(
                                                x_0,
                                                x_,
                                                eps_,
                                                eps_prev_,
                                            ...
                                            if LG.ADAIN_NOISE_MODE == "smart":
                                                if VE_MODEL:
                                                    z_[: NS.s_.shape[0], ...] = (x_ - data_)[
                                                        : NS.s_.shape[0], ...
                                                    ] / NS.s_.view(-1, *[1] * (x_.ndim - 1))
                                                else:
                                                    z_[: NS.s_.shape[0], ...] = (
                                                        x_[: NS.s_.shape[0], ...]
                                                        - (NS.sigma_max - NS.s_.view(-1, *[1] * (x_.ndim - 1)))
                                                        * data_[: NS.s_.shape[0], ...]
                                                    )[: NS.s_.shape[0], ...] / NS.s_.view(-1, *[1] * (x_.ndim - 1))
                                                RK.update_transformer_options({"z_": z_})
                        ...
                        if (
                            ...
                            and row == 0
                            and (
                                EO("implicit_lazy_recycle_first_model_call_at_start")
                                or EO("radaucycle")
                                or RK.C[0] == 0.0
                            )
                        ):
                            ...
                            x_, eps_ = RK.newton_iter(
                                ...
                                "pre",
                                SYNC_GUIDE_ACTIVE,
                            )  # will this do anything? not x_tmp

                            # DETAIL BOOST
                            if (
                                noise_scaling_type == "model_alpha"
                                and noise_scaling_weight != 0
                                and noise_scaling_eta > 0
                            ):
                            ...
                            # SYNC GUIDE ---------------------------
                            if LG.guide_mode.startswith("sync") and (
                                LG.lgw[step_sched] == 0
                                and LG.lgw_inv[step_sched] == 0
                                and LG.lgw_sync[step_sched] == 0
                                and LG.lgw_sync_inv[step_sched] == 0
                            ):
                                data_cached = None
                                ...
                                lgw_mask_drift_x_, lgw_mask_drift_x_inv_ = LG.get_masks_for_step(
                                    step_sched, lgw_type="drift_x"
                                )
                                lgw_mask_drift_y_, lgw_mask_drift_y_inv_ = LG.get_masks_for_step(
                                    step_sched, lgw_type="drift_y"
                                )
                                lgw_mask_lure_x_, lgw_mask_lure_x_inv_ = LG.get_masks_for_step(
                                    step_sched, lgw_type="lure_x"
                                )
                                lgw_mask_lure_y_, lgw_mask_lure_y_inv_ = LG.get_masks_for_step(
                                    step_sched, lgw_type="lure_y"
                                )
                                ...
                                if eps_x_ is None:
                                    ...
                                    data_x_ = torch.zeros(
                                        RK.rows + 2, *x.shape, dtype=default_dtype, device=work_device
                                    )
                                    eps_y2x_ = torch.zeros(
                                        RK.rows + 2, *x.shape, dtype=default_dtype, device=work_device
                                    )
                                    eps_x2y_ = torch.zeros(
                                        RK.rows + 2, *x.shape, dtype=default_dtype, device=work_device
                                    )
                                    eps_yt_ = torch.zeros(
                                        RK.rows + 2, *x.shape, dtype=default_dtype, device=work_device
                                    ...
                                    if noise_bongflow is None:
                                        ...
                                        noise_bongflow = normalize_zscore(
                                            NS.noise_sampler(sigma=sigma, sigma_next=NS.sigma_min),
                                            channelwise=True,
                                            inplace=True,
                                        )
                                        ...
                                        _, _ = RK(
                                            ...
                                            noise_bongflow,
                                            sigma / sigma,
                                            transformer_options={"latent_type": "xt"},
                                        )

                                        if (
                                            RK.extra_args["model_options"]["transformer_options"].get(
                                                "y0_standard_guide"
                                            )
                                            is not None
                                        ):
                                            if hasattr(
                                                model.inner_model.inner_model.diffusion_model, "y0_standard_guide"
                                            ):
                                                LG.y0 = y0_standard_guide = (
                                                    model.inner_model.inner_model.diffusion_model.y0_standard_guide.clone()
                                                )
                                        ...
                                        if (
                                            ...
                                            if hasattr(
                                                model.inner_model.inner_model.diffusion_model, "y0_inv_standard_guide"
                                            ):
                                                LG.y0_inv = y0_inv_standard_guide = (
                                                    model.inner_model.inner_model.diffusion_model.y0_inv_standard_guide.clone()
                                                )  # RK.extra_args['model_options']['transformer_options'].get('y0_standard_guide')
                                        ...
                                        y0_bongflow = (
                                            LG.HAS_LATENT_GUIDE * LG.mask * LG.y0
                                            + LG.HAS_LATENT_GUIDE_INV * LG.mask_inv * LG.y0_inv
                                        )  # LG.y0.clone()
                                    ...
                                    y0_bongflow_orig = (
                                        y0_bongflow.clone() if y0_bongflow_orig is None else y0_bongflow_orig
                                    )
                                    y0_bongflow = (
                                        y0_bongflow
                                    ...
                                    if torch.norm(y0_bongflow_orig - y0_bongflow) != 0 and EO(
                                        "enable_y0_bongflow_update"
                                    ):
                                ...
                                if (
                                    (
                                        LG.lgw[step_sched].item() in {1, 0}
                                        and LG.lgw_inv[step_sched].item() in {1, 0}
                                        and LG.lgw[step_sched] == 1 - LG.lgw_sync[step_sched]
                                        and LG.lgw_inv[step_sched] == 1 - LG.lgw_sync_inv[step_sched]
                                    )
                                    or EO("sync_speed_mode")
                                ) and not EO("disable_sync_speed_mode"):
                                    data_y = y0_bongflow.clone()
                                    eps_y = RK.get_eps(yt_0, yt_[row], data_y, sigma, s_tmp)

                                else:
                                    eps_y, data_y = RK(
                                        yt_[row], s_tmp, yt_0, sigma, transformer_options={"latent_type": "yt"}
                                    )
                                ...
                                eps_x, data_x = RK(
                                    ...
                                    x_0,
                                    sigma,
                                    transformer_options={"latent_type": "xt", "row": row, "x_tmp": x_tmp},
                                )
                                ...
                                for sync_lure_iter in range(LG.sync_lure_iter):
                                    if LG.sync_lure_sequence == "x -> y":
                                        if lure_x_mask.abs().sum() > 0:
                                            ...
                                            eps_x_lure, data_x_lure = RK(
                                                x_tmp, s_tmp, x_0, sigma, transformer_options={"latent_type": "xt"}
                                            )
                                        ...
                                        if lure_y_mask.abs().sum() > 0:
                                            ...
                                            eps_y_lure, data_y_lure = RK(
                                                y_tmp, s_tmp, yt_0, sigma, transformer_options={"latent_type": "yt"}
                                            )
                                ...
                                if RK.EXPONENTIAL:
                                    if VE_MODEL:  # ZERO IS THIS                      # ONE IS THIS
                                        ...
                                        eps_[row] = (
                                            ...
                                            + (1 - sync_mask) * eps_x2y
                                            + weight_mask * (-eps_y + sigma * (y0_bongflow - noise_bongflow))
                                        )  # +   lure_x_mask * sigma*(data_y - data_x)
                                        if EO("sync_x2y"):
                                            eps_[row] = (
                                                sync_mask * eps_x
                                                - (1 - sync_mask) * eps_x2y
                                        ...
                                        eps_yt_[row] = (
                                            sync_mask * eps_y
                                            + (1 - sync_mask) * eps_y2x
                                            + weight_mask * (-eps_x + sigma * (y0_bongflow - noise_bongflow))
                                        )  # differentiate guide as well toward the x pred?
                                else:
                                    if VE_MODEL:
                                        eps_[row] = (
                                            sync_mask * eps_x
                                            + (1 - sync_mask) * eps_x2y
                                            + weight_mask * (noise_bongflow - eps_y)
                                        )
                                        if EO("sync_x2y"):
                                            eps_[row] = (
                                                sync_mask * eps_x
                                                + (1 - sync_mask) * eps_x2y
                                                + weight_mask * (noise_bongflow - eps_x2y)
                                            )
                                    else:
                                        eps_[row] = (
                                            sync_mask * eps_x
                                            + (1 - sync_mask) * eps_x2y
                                            + weight_mask * (noise_bongflow - eps_y - y0_bongflow)
                                        )
                                        if EO("sync_x2y"):
                                            eps_[row] = (
                                                sync_mask * eps_x
                                                + (1 - sync_mask) * eps_x2y
                                                + weight_mask * (noise_bongflow - eps_x2y - y0_bongflow)
                                            )
                                        eps_yt_[row] = (
                                            sync_mask * eps_y
                                            + (1 - sync_mask) * eps_y2x
                                            + weight_mask * (noise_bongflow - eps_x - y0_bongflow)
                                        )  # differentiate guide as well toward the x pred?

                                if VE_MODEL:
                                    data_[row] = (
                                        x_0
                                        + sync_mask * NS.h * eps_x
                                        + (1 - sync_mask) * NS.h * eps_x2y
                                        - weight_mask * (sigma * (eps_y + noise_bongflow))
                                    )  # -   lure_x_mask * (sigma*(eps_y + eps_x))
                                    data_barf_y = (
                                        yt_0
                                        + sync_mask * NS.h * eps_y
                                        + (1 - sync_mask) * NS.h * eps_y2x
                                        - weight_mask * (sigma * (eps_x + noise_bongflow))
                                    )
                                    if EO("sync_x2y"):
                                        data_[row] = (
                                            x_0
                                            + sync_mask * NS.h * eps_x
                                            + (1 - sync_mask) * NS.h * eps_x2y
                                            - weight_mask * (sigma * (eps_x2y + noise_bongflow))
                                        )
                                    ...
                                    data_[row] = (
                                        ...
                                        + sync_mask * NS.h * eps_x
                                        + (1 - sync_mask) * NS.h * eps_x2y
                                        - weight_mask * (NS.h * eps_y + sigma * (noise_bongflow - y0_bongflow))
                                    )
                                    data_barf_y = (
                                        yt_0
                                        + sync_mask * NS.h * eps_y
                                        + (1 - sync_mask) * NS.h * eps_y2x
                                        - weight_mask * (NS.h * eps_x + sigma * (noise_bongflow - y0_bongflow))
                                    )
                                    if EO("sync_x2y"):
                                        data_[row] = (
                                            x_0
                                            + sync_mask * NS.h * eps_x
                                            + (1 - sync_mask) * NS.h * eps_x2y
                                            - weight_mask * (NS.h * eps_x2y + sigma * (noise_bongflow - y0_bongflow))
                                        )
                                ...
                                if EO("eps_is_y0_with_lure_x_mask"):
                                    if RK.EXPONENTIAL:
                                        ...
                                        eps_[row] = eps_[row] + lure_x_mask * (
                                            ((x_0 - y0_bongflow) / sigma) - eps_[row]
                                        )
                                data_barf = data_[row]
                                data_cached = data_x
                                ...
                                LG.guide_mode.startswith("flow")
                                and (LG.lgw[step_sched] > 0 or LG.lgw_inv[step_sched] > 0)
                                and not FLOW_STOPPED
                                and not EO("flow_sync")
                            ):
                                lgw_mask_, lgw_mask_inv_ = LG.get_masks_for_step(step)
                                if not FLOW_STARTED and not FLOW_RESUMED:
                                    ...
                                    if EO("flow_slerp"):
                                        y0_inv = (
                                            LG.HAS_LATENT_GUIDE * LG.mask * LG.y0_inv
                                            + LG.HAS_LATENT_GUIDE_INV * LG.mask_inv * LG.y0
                                        )
                                        y0 = LG.y0.clone()
                                        ...
                                        if EO("flow_manual_masks"):
                                            y0 = (
                                                (
                                                    1
                                                    - (
                                                        LG.HAS_LATENT_GUIDE * LG.lgw[step_sched] * LG.mask
                                                        + LG.HAS_LATENT_GUIDE_INV * LG.lgw_inv[step_sched] * LG.mask_inv
                                                    )
                                                )
                                                * denoised
                                                + LG.HAS_LATENT_GUIDE * LG.lgw[step_sched] * LG.mask * LG.y0
                                                + LG.HAS_LATENT_GUIDE_INV
                                                * LG.lgw_inv[step_sched]
                                                * LG.mask_inv
                                                * LG.y0_inv
                                            )
                                        else:
                                            y0 = (
                                                (1 - (lgw_mask_ + lgw_mask_inv_)) * denoised
                                                + lgw_mask_ * LG.y0
                                                + lgw_mask_inv_ * LG.y0_inv
                                            )
                                        yx0 = y0.clone()
                                        ...
                                        if EO("flow_slerp"):
                                            if EO("flow_manual_masks"):
                                                y0_inv = (
                                                    ...
                                                    * denoised
                                                    + LG.HAS_LATENT_GUIDE * LG.lgw[step_sched] * LG.mask * LG.y0_inv
                                                    + LG.HAS_LATENT_GUIDE_INV
                                                    * LG.lgw_inv[step_sched]
                                                    * LG.mask_inv
                                                    * LG.y0
                                    ...
                                    if EO("flow_manual_masks"):
                                        yx0 = (
                                            ...
                                            * yx0_prev
                                            + LG.HAS_LATENT_GUIDE * LG.lgw[step_sched] * LG.mask * x_tmp
                                            + LG.HAS_LATENT_GUIDE_INV * LG.lgw_inv[step_sched] * LG.mask_inv * x_tmp
                                        )
                                    else:
                                        yx0 = (1 - (lgw_mask_ + lgw_mask_inv_)) * yx0_prev + (
                                            lgw_mask_ + lgw_mask_inv_
                                        ) * x_tmp
                                    ...
                                    if not EO("flow_static_guides"):
                                        if EO("flow_manual_masks"):
                                            ...
                                            y0 = (
                                                (1 - (lgw_mask_ + lgw_mask_inv_)) * yx0_prev
                                                + lgw_mask_ * LG.y0
                                                + lgw_mask_inv_ * LG.y0_inv
                                            )
                                ...
                                if step == 0:
                                    noise_yt = noise_fn(
                                        y0, sigma, sigma_next, NS.noise_sampler, flow_cossim_iter
                                    )  # normalize_zscore(NS.noise_sampler(sigma=sigma, sigma_next=sigma_next), channelwise=True, inplace=True)
                                if not EO("flow_disable_renoise_y0"):
                                    if noise_yt is None:
                                        noise_yt = noise_fn(x_0, sigma, sigma_next, NS.noise_sampler, flow_cossim_iter)
                                    else:
                                        noise_yt = (1 - eta) * noise_yt + eta * noise_fn(
                                            x_0, sigma, sigma_next, NS.noise_sampler, flow_cossim_iter
                                        )
                                ...
                                if EO("flow_slerp"):
                                    ...
                                    if not EO("flow_disable_renoise_x_0"):
                                        if noise_xt is None:
                                            ...
                                            noise_xt = (1 - eta_substep) * noise_xt + eta_substep * noise_fn(
                                                x_0, sigma, sigma_next, NS.noise_sampler, flow_cossim_iter
                                            )
                                    ...
                                    if VE_MODEL:
                                        ...
                                        x_0_noised = (
                                            x_0 + (sigma / NS.sigma_max) * (noise_xt - y0)
                                        )  # just lerp noise add, (1-sigma)*y0 + sigma*noise assuming x_0 == y0, which is true initially...

                                eps_y, data_y = RK(
                                    yt, s_tmp, y0_noised, sigma, transformer_options={"latent_type": "yt"}
                                )
                                eps_x, data_x = RK(
                                    xt, s_tmp, x_0_noised, sigma, transformer_options={"latent_type": "xt"}
                                )

                                if EO("flow_slerp"):
                                    eps_y_inv, data_y_inv = RK(
                                        yt_inv,
                                        s_tmp,
                                        y0_noised_inv,
                                        sigma,
                                        transformer_options={"latent_type": "yt_inv"},
                                    )

                                if (
                                    LG.lgw[step + 1] == 0 and LG.lgw_inv[step + 1] == 0
                                ):  # break out of differentiating x0 and return to differentiating eps/velocity field
                                    ...
                                    if not EO("flow_shit_out_new"):
                                        ...
                                        eps_[row] = (1 - (lgw_mask_ + lgw_mask_inv_)) * eps_x + (
                                            lgw_mask_ + lgw_mask_inv_
                                        ) * eps_y
                                        data_[row] = (1 - (lgw_mask_ + lgw_mask_inv_)) * data_x + (
                                            lgw_mask_ + lgw_mask_inv_
                                        ) * data_y
                                        if row == 0:
                                            x_[row] = x_0 = (1 - (lgw_mask_ + lgw_mask_inv_)) * xt + (
                                                lgw_mask_ + lgw_mask_inv_
                                            ) * yt
                                        else:
                                            x_[row] = (1 - (lgw_mask_ + lgw_mask_inv_)) * xt + (
                                                lgw_mask_ + lgw_mask_inv_
                                            ) * yt
                                    ...
                                    if not EO("flow_slerp"):
                                        ...
                                        if EO("flow_reverse_data_masks"):
                                            data_[row] = (1 - (lgw_mask_ + lgw_mask_inv_)) * data_y + (
                                                lgw_mask_ + lgw_mask_inv_
                                            ) * data_x

                                        if flow_sync_eps != 0.0:
                                            if RK.EXPONENTIAL:
                                                eps_[row] = (1 - flow_sync_eps) * eps_[row] + flow_sync_eps * (
                                                    data_[row] - x_0
                                                )
                                            else:
                                                eps_[row] = (1 - flow_sync_eps) * eps_[row] + flow_sync_eps * (
                                        ...
                                        if EO("flow_sync_eps_mask"):
                                            ...
                                            if RK.EXPONENTIAL:
                                                eps_[row] = (lgw_mask_ + lgw_mask_inv_) * (1 - flow_sync_eps) * eps_[
                                                    row
                                                ] + (1 - (lgw_mask_ + lgw_mask_inv_)) * flow_sync_eps * (
                                                    data_[row] - x_0
                                                )
                                            else:
                                                eps_[row] = (lgw_mask_ + lgw_mask_inv_) * (1 - flow_sync_eps) * eps_[
                                        ...
                                        if EO("flow_sync_eps_revmask"):
                                            ...
                                            if RK.EXPONENTIAL:
                                                eps_[row] = (1 - (lgw_mask_ + lgw_mask_inv_)) * (
                                                    1 - flow_sync_eps
                                                ) * eps_[row] + (lgw_mask_ + lgw_mask_inv_) * flow_sync_eps * (
                                                    data_[row] - x_0
                                                )
                                            else:
                                                eps_[row] = (1 - (lgw_mask_ + lgw_mask_inv_)) * (
                                        ...
                                        if EO("flow_sync_eps_maskonly"):
                                            ...
                                            if RK.EXPONENTIAL:
                                                eps_[row] = (lgw_mask_ + lgw_mask_inv_) * eps_[row] + (
                                                    1 - (lgw_mask_ + lgw_mask_inv_)
                                                ) * (data_[row] - x_0)
                                            else:
                                                eps_[row] = (lgw_mask_ + lgw_mask_inv_) * eps_[row] + (
                                                    1 - (lgw_mask_ + lgw_mask_inv_)
                                        ...
                                        if EO("flow_sync_eps_revmaskonly"):
                                            ...
                                            if RK.EXPONENTIAL:
                                                eps_[row] = (1 - (lgw_mask_ + lgw_mask_inv_)) * eps_[row] + (
                                                    lgw_mask_ + lgw_mask_inv_
                                                ) * (data_[row] - x_0)
                                            else:
                                                eps_[row] = (1 - (lgw_mask_ + lgw_mask_inv_)) * eps_[row] + (
                                    ...
                                    if EO("flow_slerp"):
                                        ...
                                        if EO("flow_slerp_similarity_ratio"):
                                            ...
                                            flow_slerp_ratio2 = find_slerp_ratio_grid(
                                                data_row,
                                                data_row_inv,
                                                LG.y0.clone(),
                                                LG.y0_inv.clone(),
                                                flow_slerp_similarity_ratio,
                                            )
                            ...
                            if step < EO("direct_pre_pseudo_guide", 0) and step > 0:
                                for i_pseudo in range(EO("direct_pre_pseudo_guide_iter", 1)):
                                    x_tmp += LG.lgw[step_sched] * LG.mask * (NS.sigma_max - s_tmp) * (
                                        LG.y0 - denoised
                                    ) + LG.lgw_inv[step_sched] * LG.mask_inv * (NS.sigma_max - s_tmp) * (
                                        LG.y0_inv - denoised
                                    )
                            ...
                            if SYNC_GUIDE_ACTIVE:
                                ...
                                (not LG.guide_mode.startswith("flow"))
                                or FLOW_STOPPED
                                or (
                                    LG.guide_mode.startswith("flow")
                                    and LG.lgw[step_sched] == 0
                                    and LG.lgw_inv[step_sched] == 0
                                )
                            ):  # (LG.guide_mode.startswith("flow") and (LG.lgw[step_sched] != 0 or LG.lgw_inv[step_sched] != 0)) or FLOW_STOPPED:
                                pass
                            elif LG.guide_mode.startswith("lure") and (
                                LG.lgw[step_sched] > 0 or LG.lgw_inv[step_sched] > 0
                            ):
                                eps_[row], data_[row] = RK(
                                    x_tmp, s_tmp, x_0, sigma, transformer_options={"latent_type": "yt"}
                                )

                            else:
                                if (
                                    EO("protoshock")
                                    and StyleMMDiT is not None
                                    and StyleMMDiT.data_shock_start_step <= step_sched < StyleMMDiT.data_shock_end_step
                                ):
                                    eps_[row], data_[row] = RK(
                                        x_tmp,
                                        s_tmp,
                                        x_0,
                                        sigma,
                                        transformer_options={"row": row, "x_tmp": x_tmp, "sigma_next": sigma_next},
                                    )
                                ...
                                if hasattr(
                                    model.inner_model.inner_model.diffusion_model, "eps_out"
                                ):  # fp64 model out override, for testing only
                                    ...
                                    if RK.EXPONENTIAL:
                                        ...
                                        eps_row = eps_out
                                    if (
                                        torch.norm(eps_row - eps_[row]) < 0.01
                                        and torch.norm(data_row - data_[row]) < 0.01
                                    ):  # if some other cfg/post-cfg func was used, detect and ignore this
                                        eps_[row] = eps_row
                                        data_[row] = data_row
                            ...
                            if (
                                RK.extra_args["model_options"]["transformer_options"].get("y0_inv_standard_guide")
                                is not None
                            ):
                                if hasattr(model.inner_model.inner_model.diffusion_model, "y0_inv_standard_guide"):
                                    ...
                                    del model.inner_model.inner_model.diffusion_model.y0_inv_standard_guide
                                    RK.extra_args["model_options"]["transformer_options"]["y0_inv_standard_guide"] = (
                                        None
                                    )

                            if LG.guide_mode.startswith("lure") and (
                                LG.lgw[step_sched] > 0 or LG.lgw_inv[step_sched] > 0
                            ):
                            ...
                            if momentum != 0.0:
                                ...
                                eps_[row] = RK.get_epsilon(
                                    x_0, x_tmp, data_[row], sigma, s_tmp
                                )  # ... why was this here??? for momentum maybe?

                            if (
                                row < RK.rows
                                and noise_scaling_weight != 0
                                and noise_scaling_type in {"sampler", "sampler_substep"}
                            ):
                                if noise_scaling_type == "sampler_substep":
                                    sub_lying_su, sub_lying_sigma, sub_lying_sd, sub_lying_alpha_ratio = (
                                        NS.get_sde_substep(
                                            NS.s_[row],
                                            NS.s_[row + RK.row_offset + RK.multistep_stages],
                                            noise_scaling_eta,
                                    ...
                                    for _ in range(noise_scaling_cycles - 1):
                                        sub_lying_su, sub_lying_sigma, sub_lying_sd, sub_lying_alpha_ratio = (
                                            NS.get_sde_substep(
                                                NS.s_[row], sub_lying_sd, noise_scaling_eta, noise_scaling_mode
                                            )
                                        )
                        ...
                        if LG.y0_mean is not None and LG.y0_mean.sum() != 0.0:
                            if EO("guide_mean_scattersort"):
                                ...
                                eps_row_mean = (
                                    eps_[row]
                                    - eps_[row].mean(dim=(-2, -1), keepdim=True)
                                    + (LG.y0_mean - x_0).mean(dim=(-2, -1), keepdim=True)
                                )
                    ...
                    if EO("exp2lin_override") and RK.EXPONENTIAL:
                        x_ = RK.update_substep(
                            ...
                            NS.h_new,
                            NS.h_new_orig,
                            lying_eps_row_factor=lying_eps_row_factor,
                            sigma=sigma,
                        )  # modifies eps_[row] if lying_eps_row_factor != 1.0
                        ...
                        if not LG.guide_mode.startswith("flow") or (
                            ...
                            if LG.ADAIN_NOISE_MODE == "smart":
                                ...
                                if VE_MODEL:
                                    ...
                                    z_[row + RK.row_offset] = (
                                        x_row_tmp - (NS.sigma_max - NS.s_[row + RK.row_offset]) * data_next
                                    ) / NS.s_[row + RK.row_offset]
                                RK.update_transformer_options({"z_": z_})
                    ...
                    if not LG.guide_mode.startswith("lure"):
                        x_[row + RK.row_offset] = LG.process_guides_data_substep(
                            x_[row + RK.row_offset], data_[row], step_sched, NS.s_[row]
                        )

                    if (
                        ((not EO("protoshock") and not EO("yoloshock")) or EO("fuckitshock"))
                        and StyleMMDiT is not None
                        and StyleMMDiT.data_shock_start_step <= step_sched < StyleMMDiT.data_shock_end_step
                    ):
                        data_wct = StyleMMDiT.apply_data_shock(data_[row])
                        if VE_MODEL:
                            ...
                            x_[row + RK.row_offset] = x_[row + RK.row_offset] + (NS.sigma_max - NS.s_[row]) * (
                                data_wct - data_[row]
                            )

                    if SYNC_GUIDE_ACTIVE:  # # # # ## # # ## # YIIIIKES ---------------------------------------------------------------------------------------------------------
                        if VE_MODEL:
                            yt_[: NS.s_.shape[0], 0] = y0_bongflow + NS.s_.view(-1, *[1] * (x.ndim - 1)) * (
                                noise_bongflow
                            )
                            yt_0 = y0_bongflow + sigma * (noise_bongflow)
                        else:
                        ...
                        if RK.EXPONENTIAL:
                            ...
                            if BONGMATH:
                                ...
                                eps_y_[: NS.s_.shape[0]] = (
                                    yt_0.repeat(NS.s_.shape[0], *[1] * (x_.ndim - 1)) - data_y_[: NS.s_.shape[0]]
                                ) / sigma  # calc exact to c0 node
                                    ...
                                    if VE_MODEL:
                                        ...
                                        if EO("sync_x2y"):
                                            eps_[ms] = (
                                                sync_mask * eps_x_[ms]
                                                + (1 - sync_mask) * eps_x2y_[ms]
                                                + weight_mask * (-eps_x2y_[ms] + sigma * (-noise_bongflow))
                                            )
                                    else:
                                        eps_[ms] = (
                                            sync_mask * eps_x_[ms]
                                            + (1 - sync_mask) * eps_x2y_[ms]
                                            + weight_mask * (-eps_y_[ms] + sigma * (y0_bongflow - noise_bongflow))
                                        )
                                        if EO("sync_x2y"):
                                            eps_[ms] = (
                                                sync_mask * eps_x_[ms]
                                                + (1 - sync_mask) * eps_x2y_[ms]
                                                + weight_mask * (-eps_x2y_[ms] + sigma * (y0_bongflow - noise_bongflow))
                                            )
                                else:
                                    if VE_MODEL:
                                        eps_[ms] = (
                                        ...
                                        eps_[ms] = (
                                            sync_mask * eps_x_[ms]
                                            + (1 - sync_mask) * eps_x2y_[ms]
                                            + weight_mask * (-eps_y_[ms] + (noise_bongflow - y0_bongflow))
                                        )
                                        if EO("sync_x2y"):
                                            eps_[ms] = (
                                                sync_mask * eps_x_[ms]
                                                + (1 - sync_mask) * eps_x2y_[ms]
                                                + weight_mask * (-eps_x2y_[ms] + (noise_bongflow - y0_bongflow))
                                            )

                    if (
                        BONGMATH
                        and NS.s_[row] > RK.sigma_min
                        and NS.h < RK.sigma_max / 2
                        and (diag_iter == implicit_steps_diag or EO("enable_diag_explicit_bongmath_all"))
                        and not EO("disable_terminal_bongmath")
                    ):
                        if step == 0 and UNSAMPLE:
                            ...
                            if sigma > 0.03:
                                ...
                                if LG.ADAIN_NOISE_MODE == "smart":
                                    if VE_MODEL:
                                        z_[: NS.s_.shape[0], ...] = (x_ - data_)[: NS.s_.shape[0], ...] / NS.s_.view(
                                            -1, *[1] * (x_.ndim - 1)
                                        )
                                    else:
                                        z_[: NS.s_.shape[0], ...] = (
            ...
            if EO("postshock") and step < EO("postshock", 10):
                eps_row, data_row = RK(
                    ...
                    x_next,
                    sigma_next,
                    transformer_options={"row": row, "x_tmp": x_next, "sigma_next": sigma_next},
                )
                if VE_MODEL:
            ...
            elif not LG.guide_mode.startswith("flow") or (
                LG.lgw[step_sched] == 0
                and LG.lgw[step + 1] == 0
                and LG.lgw_inv[step_sched] == 0
                and LG.lgw_inv[step + 1] == 0
            ...
            if (
                LG.lgw[step_sched] > 0
                and step >= EO("guide_cutoff_start_step", 0)
                and cossim_counter < EO("guide_cutoff_max_iter", 10)
                and (EO("guide_cutoff") or EO("guide_min"))
            ):
        ...
        if FLOW_STARTED and not FLOW_STOPPED:
            data_x_prev_[0] = (
                data_cached  # data_cached is data_x from flow mode. this allows multistep to resume seamlessly.
            )
        ...
        for ms in range(recycled_stages):
            data_prev_[recycled_stages - ms] = data_prev_[
                recycled_stages - ms - 1
            ]  # TODO: verify that this does not run on every substep...
        ...
        rk_type = RK.swap_rk_type_at_step_or_threshold(
            x_0, data_prev_, NS, sigmas, step, rk_swap_step, rk_swap_threshold, rk_swap_type, rk_swap_print
        )
        ...
        if (
            LG.lgw[step_sched] > 0
            and step >= EO("guide_step_cutoff_start_step", 0)
            and cossim_counter < EO("guide_step_cutoff_max_iter", 10)
            and (EO("guide_step_cutoff") or EO("guide_step_min"))
        ):
    ...
    if (
        not (UNSAMPLE and sigmas[1] > sigmas[0])
        and not EO("preview_last_step_always")
        and sigma is not None
        and not (FLOW_STARTED and not FLOW_STOPPED)
    ):
        callback_step = len(sigmas) - 1 - step if sampler_mode == "unsample" else step
        preview_callback(
            x,
...
def preview_callback(
    ...
    callback(
        {"x": x, "i": step, "sigma": sigma, "sigma_next": sigma_next, "denoised": denoised_callback.to(torch.float32)}
