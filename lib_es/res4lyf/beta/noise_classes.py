...
# Source: https://github.com/ClownsharkBatwing/RES4LYF/blob/119679d8d8d26e6db52757e705488abb6399d7d4/beta/noise_classes.py
...
class BrownianNoiseGenerator(NoiseGenerator):
    def __call__(self, *, sigma=None, sigma_next=None, **kwargs):
        return BrownianTreeNoiseSampler(
            self.x, self.sigma_min, self.sigma_max, seed=self.seed, cpu=self.device.type == "cpu"
        )(sigma, sigma_next)
...
class FractalNoiseGenerator(NoiseGenerator):
    ...
    def __call__(self, *, alpha=None, k=None, scale=None, **kwargs):
        ...
        if len(self.size) == 5:
            ...
            freq = torch.sqrt(
                t_freq[:, None, None] ** 2 + y_freq[None, :, None] ** 2 + x_freq[None, None, :] ** 2
            ).clamp(min=1e-10)
        else:
...
class HiresPyramidNoiseGenerator(NoiseGenerator):
    ...
    def __call__(self, *, discount=None, mode=None, **kwargs):
        ...
        noise = (
            (
                torch.rand(
                    size=self.size, dtype=self.dtype, layout=self.layout, device=self.device, generator=self.generator
                )
                - 0.5
            )
            * 2
        ...
        for i in range(4):
            ...
            if len(self.size) == 5:
                ...
                new_noise = torch.randn(
                    (b, c, t, h, w), dtype=self.dtype, layout=self.layout, device=self.device, generator=self.generator
                )
            else:
                new_noise = torch.randn(
                    (b, c, h, w), dtype=self.dtype, layout=self.layout, device=self.device, generator=self.generator
                )
...
class CascadeBPyramidNoiseGenerator(NoiseGenerator):
    ...
    def __call__(self, *, levels=10, mode="nearest", size_range=[1, 16], **kwargs):
        ...
        epsilon = torch.randn(
            self.size, dtype=self.dtype, layout=self.layout, device=self.device, generator=self.generator
        )
        multipliers = [1]
        for i in range(1, levels):
            ...
            if size_range is None or (size_range[0] <= h <= size_range[1] or size_range[0] <= w <= size_range[1]):
                offset = torch.randn(
                    epsilon.size(0), epsilon.size(1), h, w, device=self.device, generator=self.generator
                )
            ...
            if h <= 1 or w <= 1:
                break
        epsilon = (
            epsilon / sum([m**2 for m in multipliers]) ** 0.5
        )  # divides the epsilon tensor by the square root of the sum of the squared multipliers.
...
class LaplacianNoiseGenerator(NoiseGenerator):
    ...
    def __call__(self, *, loc=None, scale=None, **kwargs):
        ...
        noise = (
            torch.randn(self.size, dtype=self.dtype, layout=self.layout, device=self.device, generator=self.generator)
            / 4.0
        )
...
class PerlinNoiseGenerator(NoiseGenerator):
    ...
    @staticmethod
    def perlin_noise_tensor(self, vectors: Tensor, positions: Tensor, step: Callable = None) -> Tensor:
        if step is None:
        ...
        if positions.shape[0] not in (1, batch_size):
            raise Exception(f"Batch sizes do not match: vectors ({vectors.shape[0]}), positions ({positions.shape[0]})")
    ...
    def perlin_noise(
        ...
        **kwargs,
        ...
        angle = torch.empty([batch_size] + [s + 1 for s in grid_shape], device=self.device, *args, **kwargs).uniform_(
            to=2.0 * pi, generator=self.generator
        )
    ...
    def __call__(self, *, detail=None, **kwargs):
        ...
        if len(self.size) == 5:
            ...
            for tt in range(t):
                for i in range(2):
                    perlin_slice = self.perlin_noise((h, w), (h, w), batch_size=c, generator=self.generator).to(
                        self.device
                    )
