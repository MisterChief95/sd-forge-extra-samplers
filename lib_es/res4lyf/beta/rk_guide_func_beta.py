...
# Source: https://github.com/ClownsharkBatwing/RES4LYF/blob/119679d8d8d26e6db52757e705488abb6399d7d4/beta/rk_guide_func_beta.py
...
try:
    ...
    def rearrange(x, pattern, **axes_lengths):
        t1 = axes_lengths["t1"]
        t2 = axes_lengths["t2"]
        if pattern == "b c (h t1) (w t2) -> b (t1 t2) c h w":
            b, c, ht1, wt2 = x.shape
            h = ht1 // t1
            w = wt2 // t2
            return x.reshape(b, c, h, t1, w, t2).permute(0, 3, 5, 1, 2, 4).reshape(b, t1 * t2, c, h, w)
        if pattern == "b (t1 t2) c h w -> b c (h t1) (w t2)":
            b, _, c, h, w = x.shape
            return x.reshape(b, t1, t2, c, h, w).permute(0, 3, 4, 1, 5, 2).reshape(b, c, h * t1, w * t2)
        raise NotImplementedError(f"Unsupported rearrange pattern in RES4LYF Forge port: {pattern}")
...
class LatentGuide:
    def __init__(
        ...
        self.lgw = torch.full_like(sigmas, 0.0, dtype=dtype)
        self.lgw_inv = torch.full_like(sigmas, 0.0, dtype=dtype)
        self.lgw_mean = torch.full_like(sigmas, 0.0, dtype=dtype)
        self.lgw_adain = torch.full_like(sigmas, 0.0, dtype=dtype)
        self.lgw_attninj = torch.full_like(sigmas, 0.0, dtype=dtype)
        self.lgw_style_pos = torch.full_like(sigmas, 0.0, dtype=dtype)
        self.lgw_style_neg = torch.full_like(sigmas, 0.0, dtype=dtype)

        self.cossim_tgt = torch.full_like(sigmas, 0.0, dtype=dtype)
        self.cossim_tgt_inv = torch.full_like(sigmas, 0.0, dtype=dtype)
    ...
    def init_guides(
        ...
        if guides is not None:
            ...
            latent_guide_weight = guides.get("weight_masked", 0.0)
            latent_guide_weight_inv = guides.get("weight_unmasked", 0.0)
            latent_guide_weight_sync = guides.get("weight_masked_sync", 0.0)
            latent_guide_weight_sync_inv = guides.get("weight_unmasked_sync", 0.0)
            latent_guide_weight_drift_x = guides.get("weight_masked_drift_x", 0.0)
            latent_guide_weight_drift_x_inv = guides.get("weight_unmasked_drift_x", 0.0)
            latent_guide_weight_drift_y = guides.get("weight_masked_drift_y", 0.0)
            latent_guide_weight_drift_y_inv = guides.get("weight_unmasked_drift_y", 0.0)
            latent_guide_weight_lure_x = guides.get("weight_masked_lure_x", 0.0)
            latent_guide_weight_lure_x_inv = guides.get("weight_unmasked_lure_x", 0.0)
            latent_guide_weight_lure_y = guides.get("weight_masked_lure_y", 0.0)
            latent_guide_weight_lure_y_inv = guides.get("weight_unmasked_lure_y", 0.0)
            latent_guide_weight_mean = guides.get("weight_mean", 0.0)
            latent_guide_weight_adain = guides.get("weight_adain", 0.0)
            latent_guide_weight_attninj = guides.get("weight_attninj", 0.0)
            latent_guide_weight_style_pos = guides.get("weight_style_pos", 0.0)
            latent_guide_weight_style_neg = guides.get("weight_style_neg", 0.0)
            ...
            self.drift_x_data = guides.get("drift_x_data", 0.0)
            self.drift_x_sync = guides.get("drift_x_sync", 0.0)
            self.drift_y_data = guides.get("drift_y_data", 0.0)
            self.drift_y_sync = guides.get("drift_y_sync", 0.0)
            self.drift_y_guide = guides.get("drift_y_guide", 0.0)
            ...
            self.guide_cossim_cutoff_ = guides.get("cutoff_masked", 1.0)
            self.guide_bkg_cossim_cutoff_ = guides.get("cutoff_unmasked", 1.0)
            self.guide_mean_cossim_cutoff_ = guides.get("cutoff_mean", 1.0)
            self.guide_adain_cossim_cutoff_ = guides.get("cutoff_adain", 1.0)
            self.guide_attninj_cossim_cutoff_ = guides.get("cutoff_attninj", 1.0)
            self.guide_style_pos_cossim_cutoff_ = guides.get("cutoff_style_pos", 1.0)
            self.guide_style_neg_cossim_cutoff_ = guides.get("cutoff_style_neg", 1.0)
            ...
            if self.guide_mode.startswith("fully_") and not RK_IMPLICIT:
                self.guide_mode = self.guide_mode[
                    6:
                ]  # fully_pseudoimplicit is only supported for implicit samplers, default back to pseudoimplicit
            ...
            if latent_guide_weights is None and scheduler_ is not None:
                ...
                latent_guide_weights = (
                    get_sigmas(self.model, scheduler_, total_steps, 1.0, shift=guide_sigma_shift).to(
            ...
            if latent_guide_weights_inv is None and scheduler_inv_ is not None:
                ...
                latent_guide_weights_inv = (
                    get_sigmas(self.model, scheduler_inv_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
            ...
            if latent_guide_weights_sync is None and scheduler_sync_ is not None:
                ...
                latent_guide_weights_sync = (
                    get_sigmas(self.model, scheduler_sync_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
            ...
            if latent_guide_weights_sync_inv is None and scheduler_sync_inv_ is not None:
                ...
                latent_guide_weights_sync_inv = (
                    get_sigmas(self.model, scheduler_sync_inv_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
                prepend = torch.zeros(start_steps_sync_inv_, dtype=self.dtype, device=self.device)
                latent_guide_weights_sync_inv = torch.cat(
                    (prepend, latent_guide_weights_sync_inv.to(self.device)), dim=0
                )
            ...
            if latent_guide_weights_drift_x is None and scheduler_drift_x_ is not None:
                ...
                latent_guide_weights_drift_x = (
                    get_sigmas(self.model, scheduler_drift_x_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
            ...
            if latent_guide_weights_drift_x_inv is None and scheduler_drift_x_inv_ is not None:
                ...
                latent_guide_weights_drift_x_inv = (
                    get_sigmas(self.model, scheduler_drift_x_inv_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
                prepend = torch.zeros(start_steps_drift_x_inv_, dtype=self.dtype, device=self.device)
                latent_guide_weights_drift_x_inv = torch.cat(
                    (prepend, latent_guide_weights_drift_x_inv.to(self.device)), dim=0
                )
            ...
            if latent_guide_weights_drift_y is None and scheduler_drift_y_ is not None:
                ...
                latent_guide_weights_drift_y = (
                    get_sigmas(self.model, scheduler_drift_y_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
            ...
            if latent_guide_weights_drift_y_inv is None and scheduler_drift_y_inv_ is not None:
                ...
                latent_guide_weights_drift_y_inv = (
                    get_sigmas(self.model, scheduler_drift_y_inv_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
                prepend = torch.zeros(start_steps_drift_y_inv_, dtype=self.dtype, device=self.device)
                latent_guide_weights_drift_y_inv = torch.cat(
                    (prepend, latent_guide_weights_drift_y_inv.to(self.device)), dim=0
                )
            ...
            if latent_guide_weights_lure_x is None and scheduler_lure_x_ is not None:
                ...
                latent_guide_weights_lure_x = (
                    get_sigmas(self.model, scheduler_lure_x_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
            ...
            if latent_guide_weights_lure_x_inv is None and scheduler_lure_x_inv_ is not None:
                ...
                latent_guide_weights_lure_x_inv = (
                    get_sigmas(self.model, scheduler_lure_x_inv_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
                prepend = torch.zeros(start_steps_lure_x_inv_, dtype=self.dtype, device=self.device)
                latent_guide_weights_lure_x_inv = torch.cat(
                    (prepend, latent_guide_weights_lure_x_inv.to(self.device)), dim=0
                )

            if latent_guide_weights_lure_y is None and scheduler_lure_y_ is not None:
                total_steps = steps_lure_y_ - start_steps_lure_y_
                latent_guide_weights_lure_y = (
                    get_sigmas(self.model, scheduler_lure_y_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
            ...
            if latent_guide_weights_lure_y_inv is None and scheduler_lure_y_inv_ is not None:
                ...
                latent_guide_weights_lure_y_inv = (
                    get_sigmas(self.model, scheduler_lure_y_inv_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
                prepend = torch.zeros(start_steps_lure_y_inv_, dtype=self.dtype, device=self.device)
                latent_guide_weights_lure_y_inv = torch.cat(
                    (prepend, latent_guide_weights_lure_y_inv.to(self.device)), dim=0
                )
            ...
            if latent_guide_weights_mean is None and scheduler_mean_ is not None:
                ...
                latent_guide_weights_mean = (
                    get_sigmas(self.model, scheduler_mean_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
            ...
            if latent_guide_weights_adain is None and scheduler_adain_ is not None:
                ...
                latent_guide_weights_adain = (
                    get_sigmas(self.model, scheduler_adain_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
            ...
            if latent_guide_weights_attninj is None and scheduler_attninj_ is not None:
                ...
                latent_guide_weights_attninj = (
                    get_sigmas(self.model, scheduler_attninj_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
            ...
            if latent_guide_weights_style_pos is None and scheduler_style_pos_ is not None:
                ...
                latent_guide_weights_style_pos = (
                    get_sigmas(self.model, scheduler_style_pos_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
                prepend = torch.zeros(start_steps_style_pos_, dtype=self.dtype, device=self.device)
                latent_guide_weights_style_pos = torch.cat(
                    (prepend, latent_guide_weights_style_pos.to(self.device)), dim=0
                )
            ...
            if latent_guide_weights_style_neg is None and scheduler_style_neg_ is not None:
                ...
                latent_guide_weights_style_neg = (
                    get_sigmas(self.model, scheduler_style_neg_, total_steps, 1.0, shift=guide_sigma_shift).to(
                        dtype=self.dtype, device=self.device
                    )
                    / self.sigma_max
                )
                prepend = torch.zeros(start_steps_style_neg_, dtype=self.dtype, device=self.device)
                latent_guide_weights_style_neg = torch.cat(
                    (prepend, latent_guide_weights_style_neg.to(self.device)), dim=0
                )
            ...
            if scheduler_inv_ != "constant":
                latent_guide_weights_inv = initialize_or_scale(
                    latent_guide_weights_inv, latent_guide_weight_inv, self.max_steps
                )
            if scheduler_sync_ != "constant":
                latent_guide_weights_sync = initialize_or_scale(
                    latent_guide_weights_sync, latent_guide_weight_sync, self.max_steps
                )
            if scheduler_sync_inv_ != "constant":
                latent_guide_weights_sync_inv = initialize_or_scale(
                    latent_guide_weights_sync_inv, latent_guide_weight_sync_inv, self.max_steps
                )

            latent_guide_weights_sync = (
                1 - latent_guide_weights_sync if latent_guide_weights_sync is not None else latent_guide_weights
            )
            latent_guide_weights_sync_inv = (
                1 - latent_guide_weights_sync_inv
                if latent_guide_weights_sync_inv is not None
                else latent_guide_weights_inv
            )
            latent_guide_weight_sync = 1 - latent_guide_weight_sync
            latent_guide_weight_sync_inv = (
                1 - latent_guide_weight_sync_inv
            )  # these are more intuitive to use if these are reversed... so that sync weight = 1.0 means "maximum guide strength"

            if scheduler_drift_x_ != "constant":
                latent_guide_weights_drift_x = initialize_or_scale(
                    latent_guide_weights_drift_x, latent_guide_weight_drift_x, self.max_steps
                )
            if scheduler_drift_x_inv_ != "constant":
                latent_guide_weights_drift_x_inv = initialize_or_scale(
                    latent_guide_weights_drift_x_inv, latent_guide_weight_drift_x_inv, self.max_steps
                )
            if scheduler_drift_y_ != "constant":
                latent_guide_weights_drift_y = initialize_or_scale(
                    latent_guide_weights_drift_y, latent_guide_weight_drift_y, self.max_steps
                )
            if scheduler_drift_y_inv_ != "constant":
                latent_guide_weights_drift_y_inv = initialize_or_scale(
                    latent_guide_weights_drift_y_inv, latent_guide_weight_drift_y_inv, self.max_steps
                )
            if scheduler_lure_x_ != "constant":
                latent_guide_weights_lure_x = initialize_or_scale(
                    latent_guide_weights_lure_x, latent_guide_weight_lure_x, self.max_steps
                )
            if scheduler_lure_x_inv_ != "constant":
                latent_guide_weights_lure_x_inv = initialize_or_scale(
                    latent_guide_weights_lure_x_inv, latent_guide_weight_lure_x_inv, self.max_steps
                )
            if scheduler_lure_y_ != "constant":
                latent_guide_weights_lure_y = initialize_or_scale(
                    latent_guide_weights_lure_y, latent_guide_weight_lure_y, self.max_steps
                )
            if scheduler_lure_y_inv_ != "constant":
                latent_guide_weights_lure_y_inv = initialize_or_scale(
                    latent_guide_weights_lure_y_inv, latent_guide_weight_lure_y_inv, self.max_steps
                )
            if scheduler_mean_ != "constant":
                latent_guide_weights_mean = initialize_or_scale(
                    latent_guide_weights_mean, latent_guide_weight_mean, self.max_steps
                )
            if scheduler_adain_ != "constant":
                latent_guide_weights_adain = initialize_or_scale(
                    latent_guide_weights_adain, latent_guide_weight_adain, self.max_steps
                )
            if scheduler_attninj_ != "constant":
                latent_guide_weights_attninj = initialize_or_scale(
                    latent_guide_weights_attninj, latent_guide_weight_attninj, self.max_steps
                )
            if scheduler_style_pos_ != "constant":
                latent_guide_weights_style_pos = initialize_or_scale(
                    latent_guide_weights_style_pos, latent_guide_weight_style_pos, self.max_steps
                )
            if scheduler_style_neg_ != "constant":
                latent_guide_weights_style_neg = initialize_or_scale(
                    latent_guide_weights_style_neg, latent_guide_weight_style_neg, self.max_steps
                )
        ...
        if latent_guide is not None:
            ...
            if type(latent_guide) is dict:
                ...
                latent_guide_samples = (
                    self.model.inner_model.inner_model.process_latent_in(latent_guide["samples"])
                    .clone()
        ...
        if latent_guide_inv is not None:
            ...
            if type(latent_guide_inv) is dict:
                ...
                latent_guide_inv_samples = (
                    self.model.inner_model.inner_model.process_latent_in(latent_guide_inv["samples"])
                    .clone()
                    .to(dtype=self.dtype, device=self.device)
                )
            ...
            if self.SAMPLE:
                ...
                x = (1 - mask_inv) * x + mask_inv * latent_guide_inv_samples.to(
                    x.device
                )  # fixed old approach, which was mask, (1-mask)
            else:
                x = latent_guide_inv_samples.to(
                    x.device
                )  # THIS COULD LEAD TO WEIRD BEHAVIOR! OVERWRITING X WITH LG_INV AFTER SETTING TO LG above!
        else:
        ...
        if latent_guide_mean is not None:
            ...
            if type(latent_guide_mean) is dict:
                ...
                latent_guide_mean_samples = (
                    self.model.inner_model.inner_model.process_latent_in(latent_guide_mean["samples"])
                    .clone()
                    .to(dtype=self.dtype, device=self.device)
                )
        ...
        if latent_guide_adain is not None:
            ...
            if type(latent_guide_adain) is dict:
                ...
                latent_guide_adain_samples = (
                    self.model.inner_model.inner_model.process_latent_in(latent_guide_adain["samples"])
                    .clone()
                    .to(dtype=self.dtype, device=self.device)
                )
        ...
        if latent_guide_attninj is not None:
            ...
            if type(latent_guide_attninj) is dict:
                ...
                latent_guide_attninj_samples = (
                    self.model.inner_model.inner_model.process_latent_in(latent_guide_attninj["samples"])
                    .clone()
                    .to(dtype=self.dtype, device=self.device)
                )
        ...
        if latent_guide_style_pos is not None:
            ...
            if type(latent_guide_style_pos) is dict:
                ...
                latent_guide_style_pos_samples = (
                    self.model.inner_model.inner_model.process_latent_in(latent_guide_style_pos["samples"])
                    .clone()
                    .to(dtype=self.dtype, device=self.device)
                )
        ...
        if latent_guide_style_neg is not None:
            ...
            if type(latent_guide_style_neg) is dict:
                ...
                latent_guide_style_neg_samples = (
                    self.model.inner_model.inner_model.process_latent_in(latent_guide_style_neg["samples"])
                    .clone()
                    .to(dtype=self.dtype, device=self.device)
                )
        ...
        if self.UNSAMPLE and not self.SAMPLE:  # sigma_next > sigma:   # TODO: VERIFY APPROACH FOR INVERSION
            if guide_inversion_y0 is not None:
                ...
                self.y0 = noise_sampler(sigma=self.sigma_max, sigma_next=self.sigma_min).to(
                    dtype=self.dtype, device=self.device
                )
            ...
            if guide_inversion_y0_inv is not None:
                ...
                self.y0_inv = noise_sampler(sigma=self.sigma_max, sigma_next=self.sigma_min).to(
                    dtype=self.dtype, device=self.device
                )
    ...
    def get_masks_for_step(self, step: int, lgw_type="default") -> Tuple[Tensor, Tensor]:
        ...
        if self.VIDEO and self.frame_weights_mgr:
            ...
            if self.HAS_LATENT_GUIDE_INV:
                frame_weights_inv = self.frame_weights_mgr.get_frame_weights_by_name(
                    "frame_weights_inv", num_frames, step
                )
    ...
    @torch.no_grad
    def process_pseudoimplicit_guides_substep(
        self,
        x_0: Tensor,
        x_: Tensor,
        eps_: Tensor,
        ...
        if not (
            lgw_mask.any() != 0 or lgw_mask_inv.any() != 0
        ):  # cossim score too similar! deactivate guide for this step
            return x_0, x_, eps_, None, None
        ...
        if self.guide_mode in {
            ...
            sub_sigma_2 = NS.sub_sigma - maxmin_ratio * (
                NS.sub_sigma
                * pseudoimplicit_row_weights[row]
                * pseudoimplicit_step_weights[full_iter]
                * self.lgw[step_sched]
            )

            eps_tmp_ = eps_.clone()

            eps_ = self.process_channelwise(
                x_0,
                eps_,
                data_,
                row,
    ...
    @torch.no_grad
    def prepare_fully_pseudoimplicit_guides_substep(
        self,
        x_0,
        x_,
        eps_,
        ...
        if "fully_pseudoimplicit" not in self.guide_mode or (
            self.lgw[step_sched] == 0 and self.lgw_inv[step_sched] == 0
        ):
            return x_0, x_, eps_
        ...
        if self.guide_mode in {
        ...
        } and (self.lgw[step_sched] > 0 or self.lgw_inv[step_sched] > 0):
            x_lying_ = x_.clone()
            ...
            for r in range(RK.rows):
                ...
                maxmin_ratio = (NS.sub_sigma - RK.sigma_min) / NS.sub_sigma
                fully_sub_sigma_2 = NS.sub_sigma - maxmin_ratio * (
                    NS.sub_sigma
                    * pseudoimplicit_row_weights[r]
                    * pseudoimplicit_step_weights[full_iter]
                    * self.lgw[step_sched]
                )
                ...
                if RK.IMPLICIT:
                    ...
                    if r > 0:
                        ...
                        if (
                            ...
                            and not self.EO("disable_fully_pseudoimplicit_bongmath")
                        ):
                            x_0, x_, eps_ = RK.bong_iter(
                                x_0,
                                x_,
                ...
                if self.HAS_LATENT_GUIDE_INV:
                    eps_substep_guide_inv = RK.get_guide_epsilon(
                        x_0, x_[r], y0_inv, sigma, NS.s_[r], NS.sigma_down, None
                    )
                ...
                eps_ = self.process_channelwise(
                    ...
                    lgw_mask,
                    lgw_mask_inv,
                    use_projection=self.guide_mode
                    in {"fully_pseudoimplicit_projection", "fully_pseudoimplicit_projection_cw"},
    ...
    @torch.no_grad
    def process_guides_data_substep(
        self,
        x_row: Tensor,
        data_row: Tensor,
        step: int,
        ...
        if self.guide_mode in {"data", "data_projection", "lure", "lure_projection"}:
            ...
            if self.HAS_LATENT_GUIDE:
                ...
                if cossim_target == 1.0:
                    ...
                    d_slerped = slerp_tensor(
                        slerp_weight, data_row, y0
                    )  # lgw_mask * slerp_weight same as using mask below
    ...
    @torch.no_grad
    def process_guides_eps_substep(
        self,
        x_0: Tensor,
        x_row: Tensor,
        data_row: Tensor,
    ...
    def process_guides_substep(
        ...
        elif self.guide_mode in {"epsilon", "epsilon_cw", "epsilon_projection", "epsilon_projection_cw"} and (
            self.lgw[step_sched] > 0 or self.lgw_inv[step_sched] > 0
        ):
            if sigma_down < sigma or s_[row] < RK.sigma_max:
                ...
                if self.HAS_LATENT_GUIDE:
                    eps_substep_guide = RK.get_guide_epsilon(
                        x_0, x_[row], y0, sigma, s_[row], sigma_down, epsilon_scale
                    )

                if self.HAS_LATENT_GUIDE_INV:
                    eps_substep_guide_inv = RK.get_guide_epsilon(
                        x_0, x_[row], y0_inv, sigma, s_[row], sigma_down, epsilon_scale
                    )
                ...
                if tol_value >= 0:
                    for b, c in itertools.product(range(x_0.shape[0]), range(x_0.shape[1])):
                        ...
                        eps_[row][b][c] = (
                            eps_[row][b][c]
                            + lgw_mask_clamp[b][0] * (eps_substep_guide[b][c] - eps_[row][b][c])
                            + lgw_mask_clamp_inv[b][0] * (eps_substep_guide_inv[b][c] - eps_[row][b][c])
                        )
                    ...
                    if self.EO("slerp_epsilon_guide"):
                        ...
                        eps_[row] = (
                            eps_[row]
                            + lgw_mask * (eps_substep_guide - eps_[row])
                            + lgw_mask_inv * (eps_substep_guide_inv - eps_[row])
                        )
                    ...
                    if self.EO("slerp_epsilon_guide"):
                        ...
                        eps_row_lerp = (
                            eps_[row]
                            + self.mask * (eps_substep_guide - eps_[row])
                            + (1 - self.mask) * (eps_substep_guide_inv - eps_[row])
                        )
        ...
        for b, c in itertools.product(range(x_0.shape[0]), range(x_0.shape[1])):
            if channelwise:
                ratio = torch.nan_to_num(
                    torch.norm(lgw_mask[b][0] * data_[row][b][c] - lgw_mask[b][0] * y0[b][c]) / avg, 0
                )
                ratio_inv = torch.nan_to_num(
                    torch.norm(lgw_mask_inv[b][0] * data_[row][b][c] - lgw_mask_inv[b][0] * y0_inv[b][c]) / avg_inv, 0
                )
            else:
                ratio = 1.0
                ratio_inv = 1.0
            ...
            if self.EO("slerp_epsilon_guide"):
                ...
                if eps_substep_guide_inv[b][c].sum() != 0:
                    eps_[row][b][c] = slerp_tensor(
                        ratio_inv * lgw_mask_inv[b][0], eps_[row][b][c], eps_substep_guide_inv[b][c]
                    )
            else:
                eps_[row][b][c] = (
                    eps_[row][b][c]
                    + ratio * lgw_mask[b][0] * (eps_substep_guide[b][c] - eps_[row][b][c])
                    + ratio_inv * lgw_mask_inv[b][0] * (eps_substep_guide_inv[b][c] - eps_[row][b][c])
                )
            ...
            if use_projection:
                if self.EO("slerp_epsilon_guide"):
                    ...
                    eps_row_lerp = (
                        eps_[row][b][c]
                        + self.mask[b][0] * (eps_substep_guide[b][c] - eps_[row][b][c])
                        + (1 - self.mask[b][0]) * (eps_substep_guide_inv[b][c] - eps_[row][b][c])
                    )  # should this ever be self.mask_inv?
                ...
                if self.EO("slerp_epsilon_guide"):
                    ...
                    eps_[row][b][c] = (
                        eps_[row][b][c]
                        + ratio * lgw_mask[b][0] * (eps_sum - eps_[row][b][c])
                        + ratio_inv * lgw_mask_inv[b][0] * (eps_sum - eps_[row][b][c])
                    )
            else:
                if self.EO("slerp_epsilon_guide"):
                    if eps_substep_guide[b][c].sum() != 0:
...
def prepare_mask(x, mask, LGW_MASK_RESCALE_MIN) -> tuple[torch.Tensor, bool]:
    ...
    if x.ndim == 5 and mask.shape[0] > 1 and mask.ndim < 4:
        ...
        spatial_mask = F.interpolate(
            spatial_mask, size=(target_frames, target_height, target_width), mode="trilinear", align_corners=False
        )  # [1, 1, F, H, W]
        ...
        spatial_mask = F.interpolate(
            spatial_mask, size=(target_height, target_width), mode="bilinear", align_corners=False
        )
...
def apply_temporal_smoothing(tensor, temporal_smoothing):
    ...
    temporal_kernel = (
        torch.tensor([0.1, 0.2, 0.4, 0.2, 0.1], device=tensor.device, dtype=tensor.dtype) * temporal_smoothing
    )
...
def noise_cossim_guide_tiled(x_list, guide, cossim_mode="forward", tile_size=2, step=0):
    ...
    x_tiled_list = [rearrange(x, "b c (h t1) (w t2) -> b (t1 t2) c h w", t1=tile_size, t2=tile_size) for x in x_list]
...
def noise_cossim_eps_tiled(x_list, eps, noise_list, cossim_mode="forward", tile_size=2, step=0):
    ...
    x_tiled_list = [rearrange(x, "b c (h t1) (w t2) -> b (t1 t2) c h w", t1=tile_size, t2=tile_size) for x in x_list]
    noise_tiled_list = [
        rearrange(noise, "b c (h t1) (w t2) -> b (t1 t2) c h w", t1=tile_size, t2=tile_size) for noise in noise_list
    ]
    ...
    noise_flat = noise_tiled_stack.view(
        noise_tiled_stack.size(0), noise_tiled_stack.size(1), -1
    )  # [n_x, n_tiles, c*h*w]
...
@torch.no_grad
def noise_cossim_guide_eps_tiled(
    x_0, x_list, y0, noise_list, cossim_mode="forward", tile_size=2, step=0, sigma=None, rk_type=None
):

    x_tiled_stack = torch.stack(
        [rearrange(x, "b c (h t1) (w t2) -> b (t1 t2) c h w", t1=tile_size, t2=tile_size)[0] for x in x_list]
    )  # [n_x, n_tiles, c, h, w]
    eps_guide_stack = torch.stack(
        [rearrange(x - y0, "b c (h t1) (w t2) -> b (t1 t2) c h w", t1=tile_size, t2=tile_size)[0] for x in x_list]
    )  # [n_x, n_tiles, c, h, w]
    del x_list
...
def handle_tiled_etc_noise_steps(
    ...
    for i in range(noise_cossim_iterations):
        ...
        x_tmp.append(
            NS.add_noise_post(
                x, sigma_up, sigma, sigma_next, alpha_ratio, s_noise, noise_mode, SDE_NOISE_EXTERNAL, sde_noise_t
            )
        )  # y0, lgw, sigma_down are currently unused
        noise_tmp = x_tmp[i] - x
    ...
    if step < EO("noise_cossim_start_step", 0):
        ...
        x = noise_cossim_eps_tiled(
            x_tmp, eps, noise_tmp_list, cossim_mode=NOISE_COSSIM_MODE, tile_size=noise_cossim_tile_size, step=step
        )
    elif NOISE_COSSIM_SOURCE == "guide_epsilon_tiled":
        x = noise_cossim_guide_eps_tiled(
            x_0,
            x_tmp,
            y0,
            noise_tmp_list,
            cossim_mode=NOISE_COSSIM_MODE,
            tile_size=noise_cossim_tile_size,
            step=step,
            sigma=sigma,
            rk_type=rk_type,
        )
        ...
        x = noise_cossim_guide_tiled(
            x_tmp, y0, cossim_mode=NOISE_COSSIM_MODE, tile_size=noise_cossim_tile_size, step=step
        )
        ...
        for i in range(len(x_tmp)):
            if (NOISE_COSSIM_MODE == "forward") and (cossim_tmp[i] == max(cossim_tmp)):
                ...
                (NOISE_COSSIM_MODE != "forward")
                and (NOISE_COSSIM_MODE != "reverse")
                and (NOISE_COSSIM_MODE != "orthogonal")
            ):
                x = x_tmp[0]
                break
    return x
